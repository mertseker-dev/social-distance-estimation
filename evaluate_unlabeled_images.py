...
def main():
    ...
    for filename in file_list:
        ...
        # focal length information is an optional input,
        # ideally, the focal length of each image should be in their metadata
        # but it can be provided manually if it's missing in the metadata
        # in that case, the user should make sure that all of the images in the given base folder have the same
        # focal lengths
        if len(sys.argv) == 7:
    ...
    if not os.path.exists('outputs_unlabeled_data'):
        os.makedirs('outputs_unlabeled_data')

    output_filename = datetime.datetime.now().strftime("%Y%m%d-%H%M%S")
    output_filename = 'outputs_unlabeled_data/' + output_filename + '.csv'

    with open(output_filename, 'w', newline='') as file:
        writer = csv.writer(file)
        writer.writerow(["Filename", "Detected Person Count", "Social Distances (mm)",
                             "Average Social Distance (mm)", "Violation Count"])
        for i in range(len(filenames)):
            writer.writerow([filenames[i], detectedPersonCounts[i], distancesStrings[i], avg_distances[i], violation_counts[i]])
        writer.writerow(['ALL', averageOfList(detectedPersonCounts), "-", averageOfList(avg_distances),
                         averageOfList(violation_counts)])
